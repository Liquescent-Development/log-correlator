...
describe('StreamJoiner', () => {
    ...
    describe('simple label mapping', () => {
      ...
      it('should fall back to right label if left not found', async () => {
        const options: StreamJoinerOptions = {
          ...defaultOptions,
          labelMappings: [
            { left: 'nonexistent_left', right: 'trace_id' }
          ]
        };
    ...
    describe('mapping edge cases', () => {
      it('should handle missing labels gracefully', async () => {
        const options: StreamJoinerOptions = {
          ...defaultOptions,
          labelMappings: [
            { left: 'nonexistent', right: 'also_nonexistent' }
          ]
        };
      ...
      it('should handle precedence by checking left key first in each event', async () => {
        const options: StreamJoinerOptions = {
          ...defaultOptions,
          labelMappings: [
            { left: 'primary_id', right: 'trace_id' }
          ]
        };
  ...
  describe('filter functionality', () => {
    describe('regex filters', () => {
      it('should filter events using regex pattern {status=~"4..|5.."}', async () => {
        ...
        expect(correlations[0].events.every(e => e.labels.status.match(/^[45]../))).toBe(true);
      });
      ...
      it('should handle complex regex patterns', async () => {
        ...
        expect(correlations).toHaveLength(1);
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.map(e => e.labels.level)).toEqual(['ERROR', 'WARN']);
      });
      ...
      it('should handle negative regex with !~', async () => {
        ...
        expect(correlations).toHaveLength(1);
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.map(e => e.labels.level)).toEqual(['INFO', 'ERROR']);
      });
    });
    ...
    describe('exact match filters', () => {
      it('should filter events using exact match {service="frontend"}', async () => {
        ...
        expect(correlations).toHaveLength(1);
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.every(e => e.labels.service === 'frontend')).toBe(true);
      });
      ...
      it('should filter events using negative exact match {service!="backend"}', async () => {
        ...
        expect(correlations).toHaveLength(1);
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.map(e => e.labels.service)).toEqual(['frontend', 'api-gateway']);
      });
    });
    ...
    describe('multiple filter conditions', () => {
      it('should apply multiple filters with AND logic', async () => {
        ...
        expect(correlations).toHaveLength(1);
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.every(e => 
    ...
    describe('filter edge cases', () => {
      ...
      it('should handle quoted values with spaces and special characters', async () => {
        ...
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].events.every(e => 
          e.labels.message === 'Error: Connection failed'
        )).toBe(true);
      });
    });
    ...
    describe('filter with other features', () => {
      it('should apply filters after ignoring clause correlation', async () => {
        ...
        expect(correlations[0].events.every(e => e.labels.level === 'ERROR')).toBe(true);
      });
  ...
  describe('integration tests and edge cases', () => {
    ...
    describe('combining all three features', () => {
      it('should handle ignoring + labelMappings + filter together', async () => {
        ...
        expect(correlations[0].events).toHaveLength(2);
        expect(correlations[0].joinValue).toBe('session123');
        expect(correlations[0].events.every(e => e.labels.level !== 'DEBUG')).toBe(true);
      });
    ...
    describe('temporal constraints with other features', () => {
      it('should apply temporal constraints with ignoring clause', async () => {
        ...
        const rightEvents = [
          createTestEvent('2025-08-13T10:00:03.000Z', { // 3 seconds later (within window)
            service: 'frontend',
            request_id: 'req123',
            instance_id: 'instance-2' // Different instance (ignored)
          }),
          createTestEvent('2025-08-13T10:00:10.000Z', { // 10 seconds later (outside window)
            service: 'frontend',
            request_id: 'req123',
      ...
      it('should apply temporal constraints with filters', async () => {
        ...
        const rightEvents = [
          createTestEvent('2025-08-13T10:00:01.000Z', { // 1 second later (within window)
            level: 'ERROR', // Should pass filter
            request_id: 'req123'
          }),
          createTestEvent('2025-08-13T10:00:01.500Z', { // 1.5 seconds later (within window)
            level: 'DEBUG', // Should be filtered out
            request_id: 'req123'
          })
        ];
    ...
    describe('grouping with other features', () => {
      ...
      it('should handle group_right with filter', async () => {
        ...
        expect(correlations.every(c => 
          c.events.every(e => e.labels.status && e.labels.status.startsWith('2'))
        )).toBe(true);
      });
    });
  ...
  describe('real-time correlation processing', () => {
    ...
    const createDelayedAsyncIterable = <T>(items: Array<{ item: T; delay: number }>): AsyncIterable<T> => {
      return {
        async *[Symbol.asyncIterator]() {
    ...
    const collectCorrelationsWithTiming = async (joiner: StreamJoiner, leftStream: AsyncIterable<LogEvent>, rightStream: AsyncIterable<LogEvent>, useRealtime = true) => {
      const results: Array<{ correlation: CorrelatedEvent; receivedAt: number }> = [];
    ...
    describe('immediate correlation emission', () => {
      ...
      it('should emit correlations immediately when matches arrive', async () => {
        ...
        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 50 }, // Matches req1
          { item: createTestEvent('2025-08-13T10:00:03Z', {}, { request_id: 'req2' }), delay: 150 }  // Matches req2
        ];
      ...
      it('should emit correlations in order of completion, not arrival order', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req1' }), delay: 10 },
          { item: createTestEvent('2025-08-13T10:00:02Z', {}, { request_id: 'req2' }), delay: 20 }
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:03Z', {}, { request_id: 'req2' }), delay: 30 }, // req2 completes first
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 80 }  // req1 completes later
        ];
      ...
      it('should handle concurrent matches for different join keys (current batch behavior)', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req1' }), delay: 10 },
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req2' }), delay: 15 },
          { item: createTestEvent('2025-08-13T10:00:02Z', {}, { request_id: 'req3' }), delay: 20 }
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:03Z', {}, { request_id: 'req2' }), delay: 25 },
          { item: createTestEvent('2025-08-13T10:00:04Z', {}, { request_id: 'req1' }), delay: 30 },
          { item: createTestEvent('2025-08-13T10:00:05Z', {}, { request_id: 'req3' }), delay: 35 }
        ];
    ...
    describe('left join real-time emission', () => {
      it('should emit partial correlations for left joins (tests current behavior)', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req1' }), delay: 10 },
          { item: createTestEvent('2025-08-13T10:00:02Z', {}, { request_id: 'req2' }), delay: 20 } // No right match
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 50 }
          // No match for req2
        ];
        ...
        const req1Result = results.find(r => r.correlation.joinValue === 'req1');
        ...
        const req2Result = results.find(r => r.correlation.joinValue === 'req2');
      ...
      it('should handle late arrivals within tolerance window', async () => {
        ...
        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 150 } // Late arrival
        ];
    ...
    describe('late-arriving events', () => {
      it('should handle late-arriving events within tolerance window', async () => {
        ...
        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 80 } // Within tolerance
        ];
      ...
      it('should reject events arriving outside tolerance window (real-time feature)', async () => {
        ...
        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 100 } // Outside tolerance
        ];
      ...
      it('should handle out-of-order event arrival with timestamps', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:02Z', {}, { request_id: 'req1' }), delay: 10 }, // Later timestamp, arrives first
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req2' }), delay: 20 }  // Earlier timestamp, arrives second
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req2' }), delay: 30 },
          { item: createTestEvent('2025-08-13T10:00:03Z', {}, { request_id: 'req1' }), delay: 40 }
        ];
        ...
        const sortedResults = results.sort((a, b) => 
          new Date(a.correlation.timestamp).getTime() - new Date(b.correlation.timestamp).getTime()
        );
    ...
    describe('performance tests', () => {
      it('should handle high-frequency event streams efficiently', async () => {
        ...
        const leftEvents = Array.from({ length: numEvents }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(Math.floor(i / 10)).padStart(2, '0')}.${String((i % 10) * 100).padStart(3, '0')}Z`, 
        ...
        const rightEvents = Array.from({ length: numEvents }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(Math.floor(i / 10)).padStart(2, '0')}.${String((i % 10) * 100 + 50).padStart(3, '0')}Z`, 
            { batch: `batch${Math.floor(i / 10)}` }, 
            { request_id: `req${i}` }
          ),
          delay: 1
        ...
        expect(results.every(r => r.correlation.metadata.completeness === 'complete')).toBe(true);
      });
      ...
      it('should handle large event streams efficiently', async () => {
        ...
        const numEvents = 200;
        const leftEvents = Array.from({ length: numEvents }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(i).padStart(3, '0')}Z`, {}, { request_id: `req${i}` }),
          delay: 1
        }));

        const rightEvents = Array.from({ length: numEvents }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(i + 300).padStart(3, '0')}Z`, {}, { request_id: `req${i}` }),
          delay: 1
        }));
      ...
      it('should handle backpressure gracefully', async () => {
        ...
        const leftEvents = Array.from({ length: 50 }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(i).padStart(2, '0')}Z`, {}, { request_id: `req${i}` }),
          delay: 1 // Fast production
        }));

        const rightEvents = Array.from({ length: 50 }, (_, i) => ({
          item: createTestEvent(`2025-08-13T10:00:${String(i + 60).padStart(2, '0')}Z`, {}, { request_id: `req${i}` }),
          delay: 1 // Fast production
        }));
        ...
        expect(results.every(r => r.metadata.completeness === 'complete')).toBe(true);
      });
    });
    ...
    describe('edge cases', () => {
      it('should handle streams ending at different times', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req1' }), delay: 10 },
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req2' }), delay: 20 }
          // Left stream ends here
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:02Z', {}, { request_id: 'req1' }), delay: 30 },
          { item: createTestEvent('2025-08-13T10:00:03Z', {}, { request_id: 'req2' }), delay: 40 },
          { item: createTestEvent('2025-08-13T10:00:04Z', {}, { request_id: 'req3' }), delay: 50 } // No left match
        ];
        ...
        expect(results.find(r => r.correlation.joinValue === 'req3')).toBeUndefined();
      });
      ...
      it('should handle streams with temporal gaps', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', {}, { request_id: 'req1' }), delay: 10 },
          // Large gap
          { item: createTestEvent('2025-08-13T10:01:00Z', {}, { request_id: 'req2' }), delay: 100 }
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:01Z', {}, { request_id: 'req1' }), delay: 50 },
          // Gap, then out-of-window event
          { item: createTestEvent('2025-08-13T10:01:05Z', {}, { request_id: 'req2' }), delay: 150 }
        ];
      ...
      it('should handle rapid successive correlations for same join key', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', { sequence: '1' }, { request_id: 'req1' }), delay: 10 },
          { item: createTestEvent('2025-08-13T10:00:01Z', { sequence: '2' }, { request_id: 'req1' }), delay: 15 },
          { item: createTestEvent('2025-08-13T10:00:02Z', { sequence: '3' }, { request_id: 'req1' }), delay: 20 }
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:03Z', { sequence: '4' }, { request_id: 'req1' }), delay: 25 },
          { item: createTestEvent('2025-08-13T10:00:04Z', { sequence: '5' }, { request_id: 'req1' }), delay: 30 }
        ];
        ...
        const timestamps = results[0].correlation.events.map((e: any) => e.timestamp);
      ...
      it('should maintain correlation accuracy under concurrent load', async () => {
        ...
        const leftEvents = [
          { item: createTestEvent('2025-08-13T10:00:00Z', { type: 'request' }, { request_id: 'req1' }), delay: 5 },
          { item: createTestEvent('2025-08-13T10:00:02Z', { type: 'request' }, { request_id: 'req2' }), delay: 8 },
          { item: createTestEvent('2025-08-13T10:00:04Z', { type: 'request' }, { request_id: 'req3' }), delay: 12 },
          { item: createTestEvent('2025-08-13T10:00:06Z', { type: 'request' }, { request_id: 'req4' }), delay: 15 }
        ];

        const rightEvents = [
          { item: createTestEvent('2025-08-13T10:00:07Z', { type: 'response' }, { request_id: 'req4' }), delay: 6 },
          { item: createTestEvent('2025-08-13T10:00:01Z', { type: 'response' }, { request_id: 'req1' }), delay: 9 },
          { item: createTestEvent('2025-08-13T10:00:05Z', { type: 'response' }, { request_id: 'req3' }), delay: 13 },
          { item: createTestEvent('2025-08-13T10:00:03Z', { type: 'response' }, { request_id: 'req2' }), delay: 16 }
        ];
        ...
        results.forEach(result => {
          ...
          const types = result.correlation.events.map((e: any) => e.labels.type).sort();
