...
class PerformanceBenchmark {
    ...
    generateCorrelatedEvents(eventCount, sourceCount) {
        ...
        for (let s = 0; s < sourceCount; s++) {
            ...
            const events = this.generateEvents(eventCount, sourceName, new Date(baseTime.getTime() + s * 50)); // Slight offset per source
    ...
    async benchmarkJoinTypes(options = {}) {
        ...
        for (const joinType of joinTypes) {
            ...
            const result = await this.runBenchmark(`${joinType.toUpperCase()} Join - ${eventCount} events`, engine, query, adapters);
    ...
    async benchmarkDataVolumes(options = {}) {
        ...
        for (const eventCount of eventCounts) {
            ...
            const result = await this.runBenchmark(`Inner Join - ${eventCount} events per source`, engine, query, adapters);
            this.results.push(result);
            this.printResult(result);
        }
    }
    ...
    async benchmarkTimeWindows(options = {}) {
        ...
        for (const timeWindow of timeWindows) {
            ...
            const result = await this.runBenchmark(`Time Window ${timeWindow} - ${eventCount} events`, engine, query, adapters);
            this.results.push(result);
            this.printResult(result);
        }
    }
    ...
    async benchmarkMultiStream(_options = {}) {
        ...
        for (const streamCount of streamCounts) {
            ...
            const result = await this.runBenchmark(`Multi-stream (${streamCount} streams) - ${eventCount} events each`, engine, query, adapters);
            this.results.push(result);
            this.printResult(result);
        }
    }
    ...
    async benchmarkMemoryUsage() {
        ...
        for (const scenario of scenarios) {
            ...
            const query = `source_0({}) and on(request_id) source_1({})`;
            const result = await this.runBenchmark(`${scenario.name} - ${scenario.eventCount} events`, engine, query, adapters);
            this.results.push(result);
            this.printResult(result);
        }
    }
    ...
    async benchmarkLateEvents() {
        ...
        for (const tolerance of lateTolerance) {
            ...
            const source2Events = this.generateEvents(eventCount, 'source_1', new Date(Date.now() + tolerance + 1000)); // Make some events late
            const adapters = new Map([
                ['source_0', new MockAdapter('source_0', source1Events)],
                ['source_1', new MockAdapter('source_1', source2Events)]
            ]);
            const query = `source_0({}) and on(request_id) source_1({})`;
            const result = await this.runBenchmark(`Late Tolerance ${tolerance}ms - ${eventCount} events`, engine, query, adapters);
            this.results.push(result);
            this.printResult(result);
        }
    }
    ...
    async stressTest() {
        ...
        const result = await this.runBenchmark(`Stress Test - ${eventCount} events × 3 sources`, engine, query, adapters);
        this.results.push(result);
        this.printResult(result);
    }
    /**
    ...
    generateReport() {
        ...
        // Top performers
        const topThroughput = this.results.reduce((max, result) => result.throughput > max.throughput ? result : max);
        const lowestMemory = this.results.reduce((min, result) => result.memoryUsage.heapUsed < min.memoryUsage.heapUsed ? result : min);
        console.log('\nTop Performers:');
