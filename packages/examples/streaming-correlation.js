...
function updateTimelineChart(correlation) {
  ...
  console.log(`   Time Range: ${correlation.timeWindow.start} to ${correlation.timeWindow.end}`);
  
  // Create ASCII timeline
  const events = correlation.events.sort((a, b) => 
...
function alertUser(correlation) {
  ...
  const errorEvents = correlation.events.filter(event => {
    const messageLower = event.message.toLowerCase();
    return messageLower.includes('error') || 
...
async function streamCorrelations() {
  ...
  try {
    ...
    for await (const correlation of engine.correlate(query)) {
      ...
      console.log(`   Streams: ${correlation.metadata.matchedStreams.join(', ')}`);
      
      // Show event details
      correlation.events.forEach((event, index) => {
        const prefix = index === 0 ? '   ├─' : '   └─';
        console.log(`${prefix} [${event.source}] ${event.timestamp}: ${event.message.substring(0, 50)}...`);
      });
  ...
  console.log(`   Complete: ${completeCorrelations} (${((completeCorrelations/totalCorrelations)*100).toFixed(1)}%)`);
  console.log(`   Partial: ${partialCorrelations} (${((partialCorrelations/totalCorrelations)*100).toFixed(1)}%)`);
  console.log(`   With Errors: ${errorCorrelations} (${((errorCorrelations/totalCorrelations)*100).toFixed(1)}%)`);
  console.log(`   Average Rate: ${(totalCorrelations / totalElapsed).toFixed(2)}/s`);
