...
class IDECorrelationEngine extends EventEmitter {
  ...
  setupAdapters(config) {
    ...
    if (config.lokiUrl) {
      this.engine.addAdapter('loki', new LokiAdapter({
        url: config.lokiUrl,
    ...
    if (config.graylogUrl) {
      this.engine.addAdapter('graylog', new GraylogAdapter({
    ...
    if (config.prometheusUrl) {
      this.engine.addAdapter('promql', new PromQLAdapter({
    ...
    return {
      ...
      events: sortedEvents.map(event => ({
        ...
        percentage: duration > 0 ? 
          ((new Date(event.timestamp).getTime() - start) / duration) * 100 : 0,
        source: event.source,
        severity: this.detectSeverity(event)
      }))
    };
  ...
  generateSummary(correlation) {
    const errorCount = correlation.events.filter(e => 
      this.detectSeverity(e) === 'error'
    ).length;
  ...
  getQuerySuggestions(partialQuery) {
    ...
    if (!partialQuery || partialQuery.length < 3) {
      suggestions.push(
        ...
        { type: 'source', value: 'graylog(', description: 'Query Graylog logs' },
        { type: 'source', value: 'promql(', description: 'Query Prometheus metrics' }
      );
    }
    ...
    if (partialQuery.includes('on(') && partialQuery.includes(')')) {
      suggestions.push(
        { type: 'modifier', value: 'within(30s)', description: 'Temporal constraint' },
        { type: 'modifier', value: 'group_left(', description: 'Many-to-one join' },
        { type: 'modifier', value: 'group_right(', description: 'One-to-many join' },
...
async function setupIDEIntegration() {
  ...
  console.log(`Found ${result.resultCount} correlations in ${result.executionTime}ms`);
