...
async function main() {
  ...
  const validQuery = 'loki({service="test"})[1m] and on(id) graylog(level:error)[1m]';
  ...
  console.log(`  Loki streams: ${lokiStreams.length > 0 ? lokiStreams.join(', ') : 'none'}`);
  
  // Example 4: Check adapter status
  console.log('\nAdapter status:');
  const adapters = correlationEngine.getAdapterStatus();
  adapters.forEach(adapter => {
    console.log(`  ${adapter.name}: ${adapter.connected ? 'connected' : 'disconnected'}`);
  });
