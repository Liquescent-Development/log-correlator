  ...
  try {
    ...
    for await (const event of correlationStream) {
      ...
      if (Date.now() - startTime > timeoutMs) {
        console.log(`  ⏱️ Timeout after ${timeoutMs}ms, collected ${events.length} events`);
        break;
      }
    }
  } catch (error: any) {
...
describeOrSkip('Graylog Correlation Engine Integration', () => {
  ...
  describe('Correlation Query Support', () => {
    it('should demonstrate how correlation queries work with the engine', async () => {
      ...
      const simpleQuery = 'graylog(tier:prd)[5m] and on(request_id) graylog(tier:prd)[5m]';

      console.log('\n📊 Testing simple correlation query:');
      console.log(`  Query: ${simpleQuery}`);

      const events = await collectCorrelationEvents(engine, simpleQuery, 5, 5000);
    ...
    it('should demonstrate correlation with join keys', async () => {
      ...
      console.log('  This query correlates all logs with request_id to error logs with matching request_id');

      const correlatedEvents = await collectCorrelationEvents(engine, correlationQuery, 5, 5000);
      ...
      if (correlatedEvents.length > 0) {
        ...
        const correlationsWithJoinKeys = correlatedEvents.filter(e =>
          e.joinKey && e.joinValue
        );
        console.log(`  📎 Correlations with join keys: ${correlationsWithJoinKeys.length}`);
        ...
        if (correlationsWithJoinKeys.length > 0) {
          ...
          console.log(`  🔑 Join key fields used: ${Array.from(joinKeyFields).join(', ')}`);
          ...
          if (firstCorrelation.events && firstCorrelation.events.length > 0) {
            console.log(`  📦 Events in first correlation: ${firstCorrelation.events.length}`);
    ...
    it('should demonstrate time window handling', async () => {
      ...
      for (const window of timeWindows) {
        ...
        console.log(`  ✅ [${window}] window: ${events.length} events retrieved`);
        ...
        if (events.length > 0) {
          ...
          if (!correlationsValid) {
            console.log(`  ⚠️ [${window}] window: Some correlations span too much time`);
            const invalidCorrelation = events.find(correlation => {
              if (!correlation.events || correlation.events.length === 0) return false;
              const eventTimes = correlation.events.map((event: any) => 
                new Date(event.timestamp).getTime()
              );
              const minTime = Math.min(...eventTimes);
            ...
            if (invalidCorrelation && invalidCorrelation.events) {
              const eventTimes = invalidCorrelation.events.map((event: any) => ({
              ...
              console.log(`    Time span: ${(maxTime - minTime) / 1000}s (max allowed: ${windowMs / 1000}s)`);
              console.log(`    Events:`, eventTimes.map((e: any) => e.time).join(', '));
            }
          }
