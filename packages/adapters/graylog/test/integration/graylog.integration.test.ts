...
describeOrSkip('Graylog Adapter Integration Tests', () => {
  ...
  describe('Simple Queries', () => {
    ...
    it('should fetch service-specific logs', async () => {
      ...
      if (config.testConfig.verbose) {
        console.log(`📊 Service-specific query returned ${events.length} events`);
        if (events.length > 0) {
  ...
  describe('Field Validation', () => {
    ...
    it('should extract correlation IDs when present', async () => {
      ...
      if (events.length > 0) {
        ...
        if (config.testConfig.verbose) {
          console.log(`📊 Found ${eventsWithCorrelation.length}/${events.length} events with correlation IDs`);
  ...
  describe('Error Handling', () => {
    it('should handle invalid queries gracefully', async () => {
      ...
      if (config.testConfig.verbose) {
        console.log('✅ Invalid query handled gracefully, returned empty array');
      }
    }, 30000); // 30 second timeout
  ...
  return new Promise<LogEvent[]>((resolve) => {
    ...
    const timeoutId = setTimeout(async () => {
      if (!settled) {
        ...
        if (config.testConfig.verbose) {
          console.log(`⏱️ Timeout after ${timeout}ms, collected ${events.length} events`);
        }
        resolve(events);
      }
    }, timeout);
        ...
        for await (const event of stream) {
          ...
          if (events.length >= max) {
            if (!settled) {
              ...
              await tempAdapter.destroy();
              if (config.testConfig.verbose) {
                console.log(`✅ Collected ${events.length} events (max reached)`);
              }
              resolve(events);
            }
            break;
          ...
          if (Date.now() - startTime > timeout) {
            if (!settled) {
              ...
              await tempAdapter.destroy();
              if (config.testConfig.verbose) {
                console.log(`⏱️ Inline timeout check at ${events.length} events`);
              }
              resolve(events);
            }
            break;
