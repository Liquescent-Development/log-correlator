...
class GraylogAdapter {
    constructor(options) {
        ...
        if (options.apiToken) {
            ...
            throw new log_correlator_core_1.CorrelationError('Graylog adapter requires either apiToken or username/password', 'AUTH_REQUIRED');
        }
    }
    getName() {
        return 'graylog';
        ...
        try {
            ...
            while (!controller.signal.aborted) {
                ...
                try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, this.options.pollInterval * 2));
                }
    ...
    async search(params, signal) {
        ...
        if (!response.ok) {
            throw new log_correlator_core_1.CorrelationError(`Graylog search failed: ${response.statusText}`, 'GRAYLOG_SEARCH_ERROR', { status: response.status });
        }
    ...
    async getAvailableStreams() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new log_correlator_core_1.CorrelationError('Failed to fetch available streams', 'GRAYLOG_STREAMS_ERROR');
            }
