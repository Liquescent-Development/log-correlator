import { DataSourceAdapter, LogEvent, CorrelationError, StreamOptions } from '@liquescent/log-correlator-core';
...
export class PromQLAdapter implements DataSourceAdapter {
  ...
  async *createStream(query: string, options?: StreamOptions): AsyncIterable<LogEvent> {
    ...
    if (!selectorMatch) {
      throw new CorrelationError('Invalid PromQL selector', 'PROMQL_PARSE_ERROR');
    }
  ...
  private parseTimeRange(timeRange: string): number {
    ...
    if (!match) {
      throw new CorrelationError('Invalid time range format', 'PROMQL_PARSE_ERROR');
    }
    ...
    switch (unit) {
      ...
      default: throw new CorrelationError('Invalid time unit', 'PROMQL_PARSE_ERROR');
    }
  }
