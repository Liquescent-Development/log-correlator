...
describe('PromQLAdapter', () => {
  ...
  describe('Authentication', () => {
    it('should create Bearer auth header from API token', async () => {
      ...
      await (() => { const iterator = streamIterator[Symbol.asyncIterator](); return iterator.next(); })();
  ...
  describe('validateQuery', () => {
    ...
    it('should validate metric selectors', () => {
      ...
      expect(adapter.validateQuery('http_requests_total{method="GET",status="200"}')).toBe(true);
    });

    it('should validate metrics with namespaces', () => {
      expect(adapter.validateQuery('prometheus:http_requests_total')).toBe(true);
  ...
  describe('Error handling', () => {
    ...
    it('should handle network errors', async () => {
      ...
      await expect((() => { const iterator = streamIterator[Symbol.asyncIterator](); return iterator.next(); })()).rejects.toThrow('Network error');
    });
    ...
    it('should handle malformed JSON responses', async () => {
      ...
      await expect((() => { const iterator = streamIterator[Symbol.asyncIterator](); return iterator.next(); })()).rejects.toThrow('Invalid JSON');
    });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle very large metric values', async () => {
      ...
      expect(result.value?.message).toBe('large_metric=1234567890123456789.123456789');
      expect(result.value?.labels.__value__).toBe('1234567890123456789.123456789');
    });
