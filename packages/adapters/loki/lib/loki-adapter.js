...
class LokiAdapter {
        ...
        while (shouldReconnect && this.reconnectAttempts < this.options.maxRetries) {
            try {
                ...
                if (this.reconnectAttempts >= this.options.maxRetries - 1) {
                    throw new log_correlator_core_1.CorrelationError('WebSocket connection failed after max retries', 'WEBSOCKET_MAX_RETRIES', { error: error instanceof Error ? error.message : String(error) });
                }
                // Calculate exponential backoff with jitter
                const delay = Math.min(baseReconnectDelay * Math.pow(2, this.reconnectAttempts) + Math.random() * 1000, maxReconnectDelay);
        ...
        try {
            ...
            while (!controller.signal.aborted) {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new log_correlator_core_1.CorrelationError(`Loki query failed: ${response.statusText}`, 'LOKI_QUERY_ERROR', { status: response.status });
                    }
                    const data = await response.json();
                    if (data.status === 'success' && data.data.result) {
                        for (const stream of data.data.result) {
                            for (const [timestamp, logLine] of stream.values) {
                                const event = this.parseLogEntry(stream.stream, { ts: timestamp, line: logLine });
                                yield event;
    ...
    async getAvailableStreams() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new log_correlator_core_1.CorrelationError('Failed to fetch available streams', 'LOKI_LABELS_ERROR');
            }
            const data = await response.json();
            return data.data || [];
        }
