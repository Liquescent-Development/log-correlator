  ...
  calculateMemoryStats() {
    ...
    return {
      heapUsed: {
        ...
        mean: heapUsed.reduce((a, b) => a + b, 0) / heapUsed.length / 1024 / 1024,
  ...
  calculateCPUStats() {
    ...
    for (let i = 1; i < this.metrics.cpuSnapshots.length; i++) {
      ...
      cpuDeltas.push({
        ...
        total: ((curr.user - prev.user) + (curr.system - prev.system)) / timeDelta / 1000
      });
    }
...
function formatReport(report) {
  ...
  output.push(`  Total Correlations: ${report.summary.totalCorrelations.toLocaleString()}`);
  output.push(`  Events/sec: ${report.summary.eventsPerSecond.toFixed(2)}`);
  output.push(`  Correlations/sec: ${report.summary.correlationsPerSecond.toFixed(2)}`);
...
async function runBenchmark() {
  ...
  const scenarios = {
    simple: () => BenchmarkScenarios.simpleCorrelation(engine, collector, CONFIG.duration),
    complex: () => BenchmarkScenarios.complexCorrelation(engine, collector, CONFIG.duration),
    throughput: () => BenchmarkScenarios.highThroughput(engine, collector, CONFIG.duration)
  };
