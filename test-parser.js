...
const { PeggyQueryParser } = require('./packages/query-parser/dist/peggy-parser');
...
const testQueries = [
  {
    ...
    query: 'loki({service="frontend"})[5m] and on(request_id) loki({service="backend"})[5m]'
  },
  {
    name: 'Temporal join',
    query: 'loki({service="frontend"})[5m] and on(request_id) within(30s) loki({service="backend"})[5m]'
  },
  {
    name: 'Label mapping',
    query: 'loki({service="auth"})[5m] and on(session_id=trace_id) loki({service="api"})[5m]'
  },
  {
    name: 'Group modifier',
    query: 'loki({service="frontend"})[5m] and on(request_id) group_left(session_id) loki({service="backend"})[5m]'
  },
  {
    name: 'Multi-stream correlation',
    query: 'loki({job="nginx"})[5m] and on(request_id) graylog(service:api)[5m] and on(request_id) loki({job="database"})[5m]'
  },
  {
    name: 'With filter',
    query: 'loki({service="frontend"})[5m] and on(request_id) loki({service="backend"})[5m] {status=~"4..|5.."}'
  }
];
...
const uglyQuery = 'loki({service="frontend"})[5m] and on(request_id) within(30s) group_left(session_id) loki({service="backend"})[5m] {status="500"}';
