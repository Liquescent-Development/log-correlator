...
**Solutions**:
```javascript
const { GraylogAdapter } = require('@liquescent/log-correlator-adapters-graylog');
...
try {
  ...
  if (error.code === 'AUTH_REQUIRED') {
    console.error('Authentication required: provide apiToken or username/password');
...
class ManagedLokiAdapter {
  ...
  async *createStream(query) {
    ...
    while (true) {
      ...
      if (!this.adapter || Date.now() - this.connectionTime > this.maxConnectionTime) {
        if (this.adapter) {
    ...
    for await (const correlation of this.engine.correlate(query)) {
      ...
      if (now - this.lastProcessTime < this.minProcessInterval) {
        await new Promise(resolve => 
          setTimeout(resolve, this.minProcessInterval - (now - this.lastProcessTime))
        );
      }
...
for await (const event of adapter.createStream('{service="frontend"}', { timeRange: '1m' })) {
...
for await (const event of adapter.createStream('service:backend', { timeRange: '5m' })) {
...
function validateConfig(options) {
  ...
  if (options.maxEvents && options.bufferSize && options.maxEvents < options.bufferSize) {
    ...
    async *correlate() {
      ...
      for await (const correlation of engine.correlate(query)) {
        ...
        if (correlationCount % 100 === 0) {
          ...
          console.log(`[DEBUG] ${correlationCount} correlations in ${elapsed}ms`);
        }
      ...
      console.log(`[DEBUG] Completed: ${correlationCount} correlations in ${totalTime}ms`);
    }
  };
}
```
